...
if (args.length < 3) {
  console.log(
    "Usage: node update.js [startdate] [username] [password] [outdir] [combine]"
  );
  exit(0);
}
...
const subjectColors = {
  ...
  Mathematik: "#ff3737",
  Geschichte: "#ce765e",
  "Ergänzungsfach Geschichte": "#ce765e",
  Chemie: "#64f7da",
  Französisch: "#e77401",
...
const getDwrSession = async () => {
  return cookieFetch(
    "https://akad.tocco.ch/nice2/dwr/call/plaincall/__System.generateId.dwr",
    {
      headers: {
        accept: "*/*",
    ...
    .then((response) =>
      getBetween(response, 'dwr.engine.remote.handleCallback("0","0","', '"')
    );
};
...
const getAppointments = async (date_from, dwrSession) => {
  ...
  const scriptSessionId =
    dwrSession +
    "/" +
    tokenify(new Date().getTime()) +
    "-" +
    tokenify(Math.random() * 10000000000000000);
  return cookieFetch(
    "https://akad.tocco.ch/nice2/dwr/call/plaincall/nice2_netui_SearchService.search.dwr",
    {
      headers: {
        accept: "*/*",
        "accept-language": "en-US,en;q=0.9,de;q=0.8",
    ...
    .then((response) => response.text())
    .then((response) =>
      getBetween(response, "returnValue:{rows:", ",sendingRows:")
    );
};
...
const update = async () => {
  ...
  eval(
    "var appointmentsData = " +
      (await getAppointments(startDate.getTime(), dwrSession))
  );
  let rooms = [];
  let teachers = [];
  let subjects = [];
  ...
  let resources = [
    {
      fieldName: "room",
      title: "Raum",
      instances: rooms.map((room) => ({ id: room, text: room })),
    },
    {
      fieldName: "teacher",
      title: "Lehrer",
      instances: teachers.map((teacher) => ({ id: teacher, text: teacher })),
    },
    {
      fieldName: "subject",
      title: "Fach",
      instances: subjects.map((subject) => ({
        id: subject,
        text: subject,
        color: getColorFromSubject(subject),
      })),
    },
  ];
  ...
  if (appointments.length > 0) {
    ...
    if (combine) {
      ...
      let origAppointments = JSON.parse(
        fs.readFileSync(dataDir + "appointments.json")
      );
      appointments = appointments.concat(
        origAppointments.filter(
          (origAppointment) =>
            appointments.findIndex((newAppointment) => {
              return (
                JSON.stringify(origAppointment) ===
                JSON.stringify(newAppointment)
              );
            }) < 0
        )
      );
      let origResources = JSON.parse(
        fs.readFileSync(dataDir + "resources.json")
      );
      resources = resources.concat(
        origResources.filter(
          (origResource) =>
            resources.findIndex((newResource) => {
              return (
                JSON.stringify(origResource) === JSON.stringify(newResource)
              );
            }) < 0
        )
      );
